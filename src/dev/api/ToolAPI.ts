...
namespace ThermalTool {
  ...
  Callback.addCallback("DestroyBlockStart", function(coords: Callback.ItemUseCoordinates, block: Tile) {
    if (MachineRegistry.isMachine(block.id)) {
      ...
      if (ThermalTool.isUseableWrench(item, 10)) {
        Network.sendToServer("Thermal.demontageMachine", { x: coords.x, y: coords.y, z: coords.z });
      }
    }
  });

  Network.addServerPacket("Thermal.demontageMachine", function(client: NetworkClient, data: Vector) {
    ...
    if (MachineRegistry.isMachine(blockID)) {
      ...
      if (ThermalTool.isUseableWrench(item, 10)) {
        ...
        ThermalTool.useWrench(item, 10, player);
      }
    }
  });
}